...
import { getRandomQuestionsByCategory,  getCategoryLabel, type Question } from "@/data/questions";
...
export default function QuizClient() {
  ...
  const [flagged, setFlagged] = useState<boolean[]>([]); // 👈 dipindah ke atas, mulai dari array kosong
  ...
  const setQuizQuestions = useQuizStore((state) => state.setQuestions);

  useEffect(() => {
    const randomQuestions = getRandomQuestionsByCategory(kategori, 20);
    setQuestions(randomQuestions);
    setQuizQuestions(randomQuestions);
    setAnswers(Array(randomQuestions.length).fill(null));
    setFlagged(Array(randomQuestions.length).fill(false));
    setCurrentQuestion(0); // 👈 reset posisi soal tiap ganti kategori
